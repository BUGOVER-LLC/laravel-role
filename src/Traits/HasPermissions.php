...
trait HasPermissions
{
    ...
    public function scopePermission(Builder $query, $permissions): Builder
    {
        ...
        return $query->where(static function ($query) use ($permissions, $rolesWithPermissions) {
            $query->whereHas('permissions', static function ($query) use ($permissions) {
                $query->where(static function ($query) use ($permissions) {
            ...
            if (count($rolesWithPermissions) > 0) {
                $query->orWhereHas('roles', static function ($query) use ($rolesWithPermissions) {
                    $query->where(static function ($query) use ($rolesWithPermissions) {
     ...
     * @throws ReflectionException
     */
    public function hasPermissionTo(int|string|Permission $permission, $guardName = null): bool
    {
        $permission_class = $this->getPermissionClass();

        if (is_string($permission)) {
            $permission = $permission_class->findByName($permission, $guardName ?? $this->getDefaultGuardName());
        }

        if (is_int($permission)) {
            $permission = $permission_class->findById($permission, $guardName ?? $this->getDefaultGuardName());
        }
    ...
    {
        return $this->load('roles', 'roles.permissions')->roles->flatMap(fn($role
        ) => $role->permissions)->sort()->values();
    }
    ...
    public function givePermissionTo(...$permissions): self
    {
        ...
        })->map->permission_id->all();

        $model = $this->getModel();

        if ($model->exists) {
            $this->getPermissionClass()->whereIn(
                'permission_id',
                $permissions
            ...
            $class::saved(static function ($object) use ($permissions, $model) {
                static $modelLastFiredOn;
                if (null !== $modelLastFiredOn && $modelLastFiredOn === $model) {
                    return;
                }
                $object->permissions()->sync($permissions, false); //@todo
    ...
    protected function getStoredPermission($permissions): PermissionModelContract
    {
        ...
        if (is_numeric($permissions)) {
            return $permission_class->findById($permissions, $this->getDefaultGuardName());
        }

        if (is_string($permissions)) {
            return $permission_class->findByName($permissions, $this->getDefaultGuardName());
        }

        if (is_array($permissions)) {
            return $permission_class->whereIn('name', $permissions)->whereIn(
                'guard_name',
                $this->getGuardNames()
            )->get();
        }
